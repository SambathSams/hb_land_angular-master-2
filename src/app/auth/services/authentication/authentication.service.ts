...
export class AuthenticationService {
  ...
  authenticate(username: any, password_encrypted: any, role: any) {
    ...
    return this.httpClient.post<any>('http://localhost:8500/api/login', { username, password_encrypted, role }).pipe(
      map(userData => {
  ...
  authenticatelogout(username: any) {
    ...
    return this.httpClient.post<any>('http://localhost:8500/api/logout', { username }).pipe(
      map(userData => {
        return userData;
      })
    );
